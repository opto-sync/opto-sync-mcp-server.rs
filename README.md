...
Read-only, organization-specific MCP diagnostics for synchronization planning,
protocol invariants, cross-language consumer compatibility, and infrastructure
posture. The final MCP `2025-11-25` protocol is exposed over stdout-pure stdio
and OAuth-protected Streamable HTTP at `/mcp`; both transports expose the exact
same tool, resource, and prompt catalog.
...
integrations behind the shared posture tools. Those two roles are deliberately
...
cargo run --locked --bin opto-sync-mcp-server

# Remote clients: Shared-Auth-protected Streamable HTTP at /mcp.
cargo run --locked --bin opto-sync-mcp-http
```
...
The Opto Sync domain layer contributes eight additional read-only tools:

- `opto_sync_consumer_matrix` — required Dart, Rust, and TypeScript features and breaking-contract policy.
- `opto_sync_contract_invariants` — stable identities, idempotent replay, causal order, watermarks, tombstones, conflicts, schemas, and telemetry invariants.
- `opto_sync_fleet_map` — exact repositories and component responsibilities.
- `opto_sync_lifecycle_state` — current formal lifecycle state, monotonic revision, and bounded transition audit.
- `opto_sync_plan` — deterministic capacity planning from a closed workload enum and bounded numeric inputs; it never executes the plan.
- `opto_sync_runtime_readiness` — exact protocol, transport, client, Shared Auth, provider-state, and lifecycle configuration.
- `opto_sync_safety_boundary` — explicit prohibited mutation, arbitrary-command, arbitrary-URL, payload, and secret surfaces.
- `opto_sync_shared_platform` — bounded context for Kubernetes, shared definitions, declarative migrations, Cloudflare, Supabase, and Fiducia.
...
The parity layer adds `contract://opto-sync/mcp-clients` and
`contract://opto-sync/providers`; the richer domain organization map wins the
intentional `orgmap://opto-sync` collision.

Domain prompts are `consumer_compatibility`, `deploy_readiness`, and
`sync_conflict_review`. Shared `dependency_review` and `provider_triage`
prompts are also present; the richer Opto Sync `deploy_readiness` prompt wins
that intentional name collision.

## Configuration

The only CLI option is the non-secret operational
`--lifecycle-audit-capacity` (`OPTO_SYNC_MCP_AUDIT_CAPACITY`, default `128`,
...
This server never applies reconciliation, writes records, changes GitHub or
cloud state, mutates Kubernetes/NATS, accepts arbitrary URLs/commands, or emits
payload/conflict contents. Secrets are neither tool inputs nor outputs and are
never logged. The eight domain tools retain explicit low-cardinality spans and
metrics from `ores-otel`; tool arguments and results are excluded. Stdout is
reserved exclusively for MCP JSON-RPC and all diagnostics go to stderr.
