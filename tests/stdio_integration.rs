...
async fn real_process_negotiates_and_lists_namespaced_tools_without_stdout_noise() {
    let mut child = tokio::process::Command::new(env!("CARGO_BIN_EXE_opto-sync-mcp-server"))
...
"
    ...
    assert!(serialized.contains("opto_sync_fleet_map"));
    assert!(serialized.contains("opto_sync_plan"));
}
