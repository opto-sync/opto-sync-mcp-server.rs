...
pub const ORGANIZATION: &str = "opto-sync";
pub const REPOSITORY: &str = "opto-sync-mcp-server.rs";
...
const REPOSITORIES: [(&str, &str); 10] = [
    ...
    (
        "syncer.rs",
        "Rust-native engine with C ABI and WebAssembly surfaces",
    ),
    (
        "opto-sync-interfaces",
        "transport-neutral schemas and generated contracts",
    ),
...
impl Workload {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ConsumerCompatibility => "consumer_compatibility",
        }
    }
}
...
pub struct PlanInput {
    ...
    /// Bounded number of operation units to evaluate.
