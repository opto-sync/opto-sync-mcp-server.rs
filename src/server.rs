...
impl OptoSyncMCPServer {
    ...
    fn opto_sync_fleet_map(&self) -> String {
    ...
    fn opto_sync_runtime_readiness(&self) -> String {
    ...
    fn opto_sync_shared_platform(&self) -> String {
        let timer = self.metrics.start(ToolClass::Inventory);
    ...
    fn opto_sync_lifecycle_state(&self) -> Result<String, String> {
        let timer = self.metrics.start(ToolClass::Health);
        let result = self
            .lifecycle
            .snapshot_and_audit()
    ...
    fn opto_sync_safety_boundary(&self) -> String {
        let timer = self.metrics.start(ToolClass::Inventory);
...
impl ServerHandler for OptoSyncMCPServer {
    ...
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions("Read-only MCP diagnostics for reconciliation, validation, protocol inspection, and consumer compatibility. The server is read-only and never logs MCP arguments or results.")
    }
}
...
mod tests {
    ...
    fn exact_prefixed_tool_catalog_is_exposed() {
        let server = OptoSyncMCPServer::new(LifecycleController::new(8).expect("valid lifecycle"));
        let names = server
            .tool_router
            .list_all()
            .into_iter()
