...
fn resolve_from(
    ...
    if argv.iter().any(|arg| arg == "--help" || arg == "-h") {
        eprintln!(
            "opto-sync-mcp-server options (stderr only; stdout is MCP JSON-RPC):\n  --lifecycle-audit-capacity  OPTO_SYNC_MCP_AUDIT_CAPACITY (8-4096)\nCredentials, provider URLs, access tokens, OTLP headers, and kubeconfig remain environment-only."
        );
        bail!("help requested");
    }
    ...
    let raw_capacity = parsed
        .flags
        .get("OPTO_SYNC_MCP_AUDIT_CAPACITY")
        .or_else(|| environment.get("OPTO_SYNC_MCP_AUDIT_CAPACITY"));
    let lifecycle_audit_capacity = match raw_capacity {
        Some(raw) => raw
            .parse::<usize>()
            .with_context(|| "OPTO_SYNC_MCP_AUDIT_CAPACITY must be an integer")?,
        None => DEFAULT_AUDIT_CAPACITY,
    };
    if !(MIN_AUDIT_CAPACITY..=MAX_AUDIT_CAPACITY).contains(&lifecycle_audit_capacity) {
        bail!(
            "OPTO_SYNC_MCP_AUDIT_CAPACITY must be between {MIN_AUDIT_CAPACITY} and {MAX_AUDIT_CAPACITY}"
        );
    }
    Ok(RuntimeConfig {
...
mod tests {
    ...
    fn operational_flag_overrides_environment_immutably() {
        let before = std::env::var_os("OPTO_SYNC_MCP_AUDIT_CAPACITY");
        let environment =
            HashMap::from([("OPTO_SYNC_MCP_AUDIT_CAPACITY".to_owned(), "64".to_owned())]);
        let resolved = resolve_from(
            &[
                "opto-sync-mcp-server".into(),
        ...
        assert_eq!(std::env::var_os("OPTO_SYNC_MCP_AUDIT_CAPACITY"), before);
    }
    ...
    fn invalid_values_unknown_options_and_secret_flags_fail_closed() {
        for arg in [
            ...
            assert!(
                resolve_from(
                    &["opto-sync-mcp-server".into(), arg.into()],
                    &config_path(),
                    &HashMap::new()
                )
                .is_err(),
    ...
    fn default_is_bounded_and_help_never_starts_the_protocol() {
        ...
        assert!(
            resolve_from(
                &["opto-sync-mcp-server".into(), "--help".into()],
                &config_path(),
                &HashMap::new()
            )
            .is_err()
